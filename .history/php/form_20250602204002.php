...
<body>
    ...
    function generateHeatmap() {
        .catch(error => {
                    ...
                    <
                    /html></body >
    </script> }); document.getElementById(id).addEventListener('input', generateHeatmap);
    ['varPermMin','varPermMax','varPersMin','varPersMax'].forEach(id => { window.addEventListener('DOMContentLoaded',
    generateHeatmap); // Generate heatmap saat halaman load dan saat input berubah } Plotly.newPlot('heatmapPlot', data,
    layout, {responsive: true}); }; autosize: true yaxis: { title: 'Permintaan' }, xaxis: { title: 'Persediaan' },
    title: 'Heat Map Permintaan-Persediaan-Produksi', const layout = { }]; colorscale: 'YlOrRd' type: 'heatmap', y:
    permintaan, x: persediaan, z: produksi, const data = [{ } } produksi[i][j] = Math.max(0, permintaan[i] +
    persediaan[j]) / 2; // Ganti rumus di bawah dengan hasil perhitungan fuzzy Sugeno jika sudah ada for (let j = 0; j <
        persediaan.length; j++) { produksi[i]=[]; for (let i=0; i < permintaan.length; i++) { // Dummy:
        produksi=permintaan - persediaan (ganti dengan hasil fuzzy Sugeno jika ada) } persediaan.push(j); for (let
        j=persMin; j <=persMax; j +=persStep) { } permintaan.push(i); for (let i=permMin; i <=permMax; i +=permStep) {
        const persStep=Math.max(1, Math.floor((persMax - persMin) / 20)); const permStep=Math.max(1, Math.floor((permMax
        - permMin) / 20)); const persMax=parseInt(document.getElementById('varPersMax').value); const
        persMin=parseInt(document.getElementById('varPersMin').value); const
        permMax=parseInt(document.getElementById('varPermMax').value); const
        permMin=parseInt(document.getElementById('varPermMin').value); // Buat grid data const produksi=[]; const
        persediaan=[]; }); }); </script>
</body>
