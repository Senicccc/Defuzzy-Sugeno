...
<body>
    ...
    ['id' => 'varPermMax', 'label' => 'Permintaan Maksimum:', 'value' => 0, 'note' => 'Nilai permintaan tertinggi yang pernah dicapai selama periode analisis.'],
    ['id' => 'varPersMin', 'label' => 'Persediaan Minimum:', 'value' => 0, 'note' => 'Jumlah stok terendah yang tersedia di gudang pada periode tertentu.'],
    ['id' => 'varPersMax', 'label' => 'Persediaan Maksimum:', 'value' => 0, 'note' => 'Jumlah stok terbanyak yang pernah tersedia di gudang.'],
    ...
    ['id' => 'varProdMax', 'label' => 'Produksi Maksimum:', 'value' => 0, 'note' => 'Jumlah produksi terbanyak yang pernah dilakukan dalam satu periode.']
                        ];
        ...
        <div class="card" style="background: #fffbe6; border-left: 4px solid #f7b731; margin-bottom: 16px;">
            ...
            <p>
                <strong>Catatan:</strong> Grafik 3D Surface di bawah ini adalah visualisasi lanjutan yang biasanya
                digunakan oleh analis atau ahli untuk eksplorasi lebih mendalam.
                Jika Anda tidak memahami grafik 3D, tidak perlu khawatir—<b>heatmap di atas sudah cukup mewakili hasil
                    hubungan antara permintaan, persediaan, dan produksi</b>.<br>
                <br>
                <em>Fitur 3D Surface hanya tersedia di halaman form penghitungan ini dan tidak muncul pada halaman
        ...
        document.getElementById('saveBtn')?.addEventListener('click', function() {
            ...
            fetch('save_calculation.php', {
                    ...
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',nt-Type': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest''
                    },
                    body: JSON.stringify(data)dy: JSON.stringify(data)
                })
                .then(response => {hen(response => {
                    if (!response.ok) {k) {
                        throw new Error('Network response was not ok');('Network response was not ok');
                    }
                    return response.json();eturn response.json();
                })
                .then(data => {hen(data => {
                    if (data.success) {ccess) {
                        alert('Perhitungan berhasil disimpan!');gan berhasil disimpan!');
                    } else {
                        alert('Gagal menyimpan: ' + (data.message || 'Terjadi kesalahan'));t('Gagal menyimpan: ' + (data.message || 'Terjadi kesalahan'));
                    }
                })
                .catch(error => {atch(error => {
                    console.error('Error:', error);('Error:', error);
                    alert('Terjadi kesalahan saat menyimpan');enyimpan');
                });
        });

        // Fungsi Heatmap        // Fungsi Heatmap
        function generateHeatmap() {Heatmap() {
            const permintaan = [];
            const persediaan = [];
            const produksi = [];

            // Buat grid data            // Buat grid data
            const permMin = parseInt(document.getElementById('varPermMin').value);arseInt(document.getElementById('varPermMin').value);
            ...
            const permStep = Math.max(1, Math.floor((permMax - permMin) / 20));            const permStep = Math.max(1, Math.floor((permMax - permMin) / 20));
            const persStep = Math.max(1, Math.floor((persMax - persMin) / 20));

            for (let i = permMin; i <= permMax; i += permStep) {            for (let i = permMin; i <= permMax; i += permStep) {
                permintaan.push(i);
            }
            for (let j = persMin; j <= persMax; j += persStep) {or (let j = persMin; j <= persMax; j += persStep) {
                persediaan.push(j);
            }

            // Dummy: produksi = (permintaan + persediaan) / 2 (ganti dengan hasil fuzzy Sugeno jika ada)            // Dummy: produksi = (permintaan + persediaan) / 2 (ganti dengan hasil fuzzy Sugeno jika ada)
            for (let i = 0; i < permintaan.length; i++) {
                produksi[i] = [];
                for (let j = 0; j < persediaan.length; j++) { < persediaan.length; j++) {
                    produksi[i][j] = Math.max(0, permintaan[i] + persediaan[j]) / 2;] + persediaan[j]) / 2;
                }
            }
            ...
            const data = [{            const data = [{
                ...
                y: permintaan,
                type: 'heatmap',',
                colorscale: 'YlOrRd'rRd'
            }];

            const layout = {            const layout = {
                title: 'Heat Map Permintaan-Persediaan-Produksi', Map Permintaan-Persediaan-Produksi',
                xaxis: {
                    title: 'Persediaan'e: 'Persediaan'
                },
                yaxis: {xis: {
                    title: 'Permintaan'e: 'Permintaan'
                },
                autosize: truetosize: true
            };

            Plotly.newPlot('heatmapPlot', data, layout, {            Plotly.newPlot('heatmapPlot', data, layout, {
                responsive: true
            });
        }

        window.addEventListener('DOMContentLoaded', generateHeatmap);        window.addEventListener('DOMContentLoaded', generateHeatmap);
        ['varPermMin', 'varPermMax', 'varPersMin', 'varPersMax'].forEach(id => {ach(id => {
            document.getElementById(id).addEventListener('input', generateHeatmap);p);
        });
        </script>cript>
</body>
