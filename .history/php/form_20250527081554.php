...
<body>
    ...
    <!-- Membership Visualization Script (Renamed) -->
    <script>
    // Membership Visualization Script (Renamed to avoid conflict)
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize membership charts
        initMembershipVisualization();

        // Add event listeners for range inputs
        const rangeInputs = ['varPermMin', 'varPermMax', 'varPersMin', 'varPersMax'];
    ...
    // Initialize membership visualization
    function initMembershipVisualization() {
        // Get range values
        ...
        createMembershipChart(
            ...
            persMax,
            ['Sedikit', 'Banyak'],
            ['rgba(241, 196, 15, 1)', 'rgba(52, 152, 219, 1)']
        );
    }

    // Create membership chart
    function createMembershipChart(canvasId, title, min, max, labels, colors) {
        ...
        for (let x = min; x <= max; x += step) {
            chartLabels.push(Math.round(x));
            const membership1 = 1 - (x - min) / (max - min);
            const membership2 = (x - min) / (max - min);

            dataset1.push(Math.max(0, Math.min(1, membership1)));
            dataset2.push(Math.max(0, Math.min(1, membership2)));
        }
    ...
    // Update membership charts when range changes
    function updateMembershipCharts() {
        // Validate inputs first
        const permMin = parseFloat(document.getElementById('varPermMin').value);
        const permMax = parseFloat(document.getElementById('varPermMax').value);
        const persMin = parseFloat(document.getElementById('varPersMin').value);
        const persMax = parseFloat(document.getElementById('varPersMax').value);

        if (permMin >= permMax || persMin >= persMax) {
            return; // Don't update if ranges are invalid
        }

        // Reinitialize charts with new ranges
        initMembershipVisualization();
    }
