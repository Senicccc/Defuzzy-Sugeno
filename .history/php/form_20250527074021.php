<?php
session_start();
require_once 'db_connection.php'; // File untuk koneksi database

// Cek status login dengan lebih aman
...
<body>
    ...
    ?>
    <main>
        <h1>Penerapan Logika Fuzzy Sugeno dalam Keputusan Jumlah Produksi</h1>
        ...
        // Fungsi keanggotaan untuk permintaan (linear turun/naik)
        function calculatePermintaanMembership(x, min, max) {
            const turun = 1 - (x - min) / (max - min);
            const naik = (x - min) / (max - min);
            return {
                turun: Math.max(0, Math.min(1, turun)),
                naik: Math.max(0, Math.min(1, naik))
            };
        }

        // Fungsi keanggotaan untuk persediaan (linear turun/naik)
        function calculatePersediaanMembership(x, min, max) {
            const sedikit = 1 - (x - min) / (max - min);
            const banyak = (x - min) / (max - min);
            return {
                sedikit: Math.max(0, Math.min(1, sedikit)),
                banyak: Math.max(0, Math.min(1, banyak))
            };
        }

        // Inisialisasi chart keanggotaan
        function initializeMembershipCharts() {
            // Dapatkan nilai range dari input
            const permMin = parseFloat(document.getElementById('varPermMin').value) || 486;
            const permMax = parseFloat(document.getElementById('varPermMax').value) || 9868;
            const persMin = parseFloat(document.getElementById('varPersMin').value) || 743;
            const persMax = parseFloat(document.getElementById('varPersMax').value) || 3761;

            // Hitung titik-titik untuk grafik
            const permStep = (permMax - permMin) / 50;
            const persStep = (persMax - persMin) / 50;
            ...
            // Generate data untuk permintaan
            for (let x = permMin; x <= permMax; x += permStep) {
                permLabels.push(Math.round(x));
                const {
                    turun,
                    naik
                } = calculatePermintaanMembership(x, permMin, permMax);
                permTurunData.push(turun);
                permNaikData.push(naik);
            }

            // Generate data untuk persediaan
            for (let x = persMin; x <= persMax; x += persStep) {
                persLabels.push(Math.round(x));
                const {
                    sedikit,
                    banyak
                } = calculatePersediaanMembership(x, persMin, persMax);
                persSedikitData.push(sedikit);
                persBanyakData.push(banyak);
            }

            // Buat grafik permintaan
            const permCtx = document.getElementById('permintaanChart').getContext('2d');
            const permintaanChart = new Chart(permCtx, {
                type: 'line',
                data: {
                    labels: permLabels,
                    datasets: [{
            ...
            // Buat grafik persediaan
            const persCtx = document.getElementById('persediaanChart').getContext('2d');
            const persediaanChart = new Chart(persCtx, {
                type: 'line',
                data: {
                    labels: persLabels,
                    datasets: [{
                            label: 'Sedikit',
                            data: persSedikitData,
            ...
            window.membershipCharts = {
                permintaan: permintaanChart,
                persediaan: persediaanChart
            };
        }

        // Update chart ketika input berubah
        function updateMembershipCharts() {
            // Dapatkan nilai range baru
            const permMin = parseFloat(document.getElementById('varPermMin').value) || 486;
            const permMax = parseFloat(document.getElementById('varPermMax').value) || 9868;
            const persMin = parseFloat(document.getElementById('varPersMin').value) || 743;
            const persMax = parseFloat(document.getElementById('varPersMax').value) || 3761;

            // Hitung titik-titik baru untuk grafik
            const permStep = (permMax - permMin) / 50;
            const persStep = (persMax - persMin) / 50;
            ...
            // Update chart permintaan
            window.membershipCharts.permintaan.data.labels = permLabels;
            window.membershipCharts.permintaan.data.datasets[0].data = permTurunData;
            window.membershipCharts.permintaan.data.datasets[1].data = permNaikData;
            window.membershipCharts.permintaan.update();

            // Update chart persediaan
            window.membershipCharts.persediaan.data.labels = persLabels;
            window.membershipCharts.persediaan.data.datasets[0].data = persSedikitData;
            window.membershipCharts.persediaan.data.datasets[1].data = persBanyakData;
            window.membershipCharts.persediaan.update();
        }

        // Inisialisasi saat halaman dimuat
        document.addEventListener('DOMContentLoaded', function() {
                    initializeMembershipCharts();

                    // Tambahkan event listener untuk update chart saat input berubah
                    const rangeInputs = [
                        'varPermMin', 'varPermMax',
                        'varPersMin', 'varPersMax'
                    ];

                    rangeInputs.forEach(id => {
                        document.getElementById(id).addEventListener('input', function() {
                            // Validasi input
                            const min = parseFloat(document.getElementById(id.replace('Max', 'Min'))
                                .value;
                                const max = parseFloat(document.getElementById(id.replace('Min',
                                        'Max')).value;
                                    ...
                                    if (min >= max) {
                                        document.getElementById(id + 'Error').textContent =
                                            ...
                                            'Nilai maksimum harus lebih besar dari minimum';
                                        return;
                                    } else {
                                        document.getElementById(id + 'Error').textContent = '';
                                    }
