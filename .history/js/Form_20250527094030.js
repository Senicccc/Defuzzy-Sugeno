...
function updateSurfacePlot() {
    ...
    if (contourPlotInstance) {lot");
        Plotly.purge("contourPlot");
    }f (contourPlotInstance) {
        Plotly.purge("contourPlot");
    // Render plot baru
    surfacePlotInstance = Plotly.newPlot(
        "surfacePlot",u
        [surfaceData],e = Plotly.newPlot(
        surfaceLayout,
    );  [surfaceData],
    contourPlotInstance = Plotly.newPlot(
        "contourPlot",
        [contourData],e = Plotly.newPlot(
        contourLayout,
    );  [contourData],
}       contourLayout
    );
...
function hitungSemua() {
    ...
    const { nilaiR1, nilaiR2, nilaiR3, nilaiR4 } = hitungAturanFuzzy(
        permTurun,run, permNaik, persSedikit, persBanyak } = hitungDerajatKeanggotaan();
        permNaik,R1, nilaiR2, nilaiR3, nilaiR4 } = hitungAturanFuzzy(
        persSedikit,
        persBanyak
    );  persSedikit,
    const { prodBerkurang, prodBertambah } = hitungHasilInferensi(
        nilaiR1,
        nilaiR2,Berkurang, prodBertambah } = hitungHasilInferensi(
        nilaiR3,
        nilaiR4,
    );  nilaiR3,
    hitungDefuzzifikasi(prodBerkurang, prodBertambah);
    updateCharts();
    updateSurfacePlot();prodBerkurang, prodBertambah);
}   updateCharts();
    updateSurfacePlot();
// Event listener untuk tombol hitung
document.getElementById("calculateBtn").addEventListener("click", hitungSemua);
// Event listener untuk tombol hitung
// Jalankan perhitungan pertama kali halaman dimuattener("click", hitungSemua);
document.addEventListener("DOMContentLoaded", function () {
    // Inisialisasi nilai default jika diperlukanat
    if (!document.getElementById("varPermMin").value) {() {
        ...
        document.getElementById("persediaanX").value = "2903";
    // Jalankan perhitungan awal dengan pre-input yang benar
    hitungSemua();
}); // Jalankan perhitungan awal dengan pre-input yang benar
    hitungSemua();
});