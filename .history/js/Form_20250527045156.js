...
function hitungDerajatKeanggotaan() {
  ...
  // Update tampilan
  document.getElementById("permTurun").textContent = Math.max(
    0,
    Math.min(1, permTurun)
  ).toFixed(4);
  document.getElementById("permNaik").textContent = Math.max(
    0,
    Math.min(1, permNaik)
  ).toFixed(4);
  document.getElementById("persSedikit").textContent = Math.max(
    0,
    Math.min(1, persSedikit)
  ).toFixed(4);
  document.getElementById("persBanyak").textContent = Math.max(
    0,
    Math.min(1, persBanyak)
  ).toFixed(4);
...
function prepareChartData() {
  ...
  for (let x = varPermMin; x <= varPermMax; x += stepPerm) {
    ...
    permInputLine.push(
      x === Math.round(permintaanX / stepPerm) * stepPerm ? 1 : null
    );
  }
  ...
  for (let x = varPersMin; x <= varPersMax; x += stepPers) {
    ...
    persInputLine.push(
      x === Math.round(persediaanX / stepPers) * stepPers ? 1 : null
    );
  }
  ...
  permintaanChart = new Chart(permintaanCtx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Nilai Input",
          data: Array(chartData.permintaan.labels.length)
            .fill(null)
            .map((_, i) => {
              return chartData.permintaan.labels[i] ===
                Math.round(chartData.permintaan.inputX)
                ? 1
                : null;
            }),
  ...
  persediaanChart = new Chart(persediaanCtx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Nilai Input",
          data: Array(chartData.persediaan.labels.length)
            .fill(null)
            .map((_, i) => {
              return chartData.persediaan.labels[i] ===
                Math.round(chartData.persediaan.inputX)
                ? 1
                : null;
            }),
          borderColor: "rgb(0, 0, 0)",
