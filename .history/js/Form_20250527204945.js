...
function hitungDerajatKeanggotaan() {
  ...
  document.getElementById("permTurun").textContent = (
    Math.max(0, Math.min(1, permTurun)) * 100
  ).toFixed(2);
  document.getElementById("permNaik").textContent = (
    Math.max(0, Math.min(1, permNaik)) * 100
  ).toFixed(2);
  document.getElementById("persSedikit").textContent = (
    Math.max(0, Math.min(1, persSedikit)) * 100
  ).toFixed(2);
  document.getElementById("persBanyak").textContent = (
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) =>
              `${ctx.dataset.label}: ${(ctx.raw * 100).toFixed(2)}%`,
          },
        },
        legend: {
          position: "top",
