...
// Fungsi untuk memperbarui chart dengan tampilan seperti di jurnal
function updateCharts() {
    ...
    const permTurunVal = Math.max(0, Math.min(1, (varPermMax - permintaanX) / (varPermMax - varPermMin)));
    const permNaikVal = Math.max(0, Math.min(1, (permintaanX - varPermMin) / (varPermMax - varPermMin)));
    const persSedikitVal = Math.max(0, Math.min(1, (varPersMax - persediaanX) / (varPersMax - varPersMin)));
    const persBanyakVal = Math.max(0, Math.min(1, (persediaanX - varPersMin) / (varPersMax - varPersMin)));
    ...
    permintaanChart = new Chart(permintaanCtx, {
        ...
        data: {
            ...
            datasets: [
                ...
                // Garis vertikal putus-putus di titik input
                {
                    label: "Input",
                    data: Array(chartData.permintaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.permintaan.labels.indexOf(Math.round(permintaanX)) ? 1 : null;
                    }),
                ...
                // Garis horizontal merah untuk nilai Turun
                {
                    label: "Turun Value",
                    data: Array(chartData.permintaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.permintaan.labels.indexOf(Math.round(permintaanX)) ? permTurunVal : null;
                    }),
                    borderColor: "red",
                    borderWidth: 1,
                    borderDash: [2, 2],
                ...
                // Garis horizontal merah untuk nilai Naik
                {
                    label: "Naik Value",
                    data: Array(chartData.permintaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.permintaan.labels.indexOf(Math.round(permintaanX)) ? permNaikVal : null;
                    }),
                    borderColor: "red",
                    borderWidth: 1,
                    borderDash: [2, 2],
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            if (context.dataset.label === "Turun") return `Turun: ${context.raw.toFixed(3)}`;
                            if (context.dataset.label === "Naik") return `Naik: ${context.raw.toFixed(3)}`;
                            return null;
                        }
                    }
                },
                legend: {
                    ...
                    labels: {
                        ...
                        filter: function(item) {
                            // Hanya tampilkan legend untuk Turun dan Naik
                            return item.text === 'Turun' || item.text === 'Naik';
                        }
                    }
                }
            },
    ...
    persediaanChart = new Chart(persediaanCtx, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: "Input",
                    data: Array(chartData.persediaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.persediaan.labels.indexOf(Math.round(persediaanX)) ? 1 : null;
                    }),
                    borderColor: "rgb(0, 0, 0)",
                    borderWidth: 1,
                    borderDash: [5, 5],
                ...
                // Garis horizontal merah untuk nilai Sedikit
                {
                    label: "Sedikit Value",
                    data: Array(chartData.persediaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.persediaan.labels.indexOf(Math.round(persediaanX)) ? persSedikitVal : null;
                    }),
                    borderColor: "red",
                    borderWidth: 1,
                    borderDash: [2, 2],
                ...
                // Garis horizontal merah untuk nilai Banyak
                {
                    label: "Banyak Value",
                    data: Array(chartData.persediaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.persediaan.labels.indexOf(Math.round(persediaanX)) ? persBanyakVal : null;
                    }),
                    borderColor: "red",
                    borderWidth: 1,
                    borderDash: [2, 2],
                ...
                // Titik nilai keanggotaan Sedikit
                {
                    label: "Sedikit Point",
                    data: Array(chartData.persediaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.persediaan.labels.indexOf(Math.round(persediaanX)) ? persSedikitVal : null;
                ...
                // Titik nilai keanggotaan Banyak
                {
                    label: "Banyak Point",
                    data: Array(chartData.persediaan.labels.length).fill(null).map((_, i) => {
                        return i === chartData.persediaan.labels.indexOf(Math.round(persediaanX)) ? persBanyakVal : null;
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            if (context.dataset.label === "Sedikit") return `Sedikit: ${context.raw.toFixed(3)}`;
                            if (context.dataset.label === "Banyak") return `Banyak: ${context.raw.toFixed(3)}`;
                            return null;
                        }
                    }
                },
                legend: {
                    ...
                    labels: {
                        ...
                        usePointStyle: true,
                        filter: function(item) {
                            // Hanya tampilkan legend untuk Sedikit dan Banyak
                            return item.text === 'Sedikit' || item.text === 'Banyak';
                        }
                    }
                }
            },
            ...
            datasets: [
                {
                    ...
                    data: [
                        parseFloat(document.getElementById("nilaiR1").textContent || 0),
                        parseFloat(document.getElementById("nilaiR2").textContent || 0),
                        parseFloat(document.getElementById("nilaiR3").textContent || 0),
                        parseFloat(document.getElementById("nilaiR4").textContent || 0)
                    ],
                    backgroundColor: [
                        "rgba(255, 99, 132, 0.7)",
                        "rgba(54, 162, 235, 0.7)",
