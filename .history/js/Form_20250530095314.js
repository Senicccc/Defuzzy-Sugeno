...
function prepareChartData() {
    ...
    // Mulai dari 0 sampai max+margin (agar ada garis di 100%)
    const permLabels = [];
    const permTurunData = [];
    const permNaikData = [];
    const permInputLine = [];
    ...
    const permStart = 0;
    const permEnd = varPermMax + (varPermMax - varPermMin) * 0.05;
    const stepPerm = (permEnd - permStart) / 100;
    for (let x = permStart; x <= permEnd; x += stepPerm) {
        permLabels.push(Math.round(x));
        permTurunData.push(
            x < varPermMin ? 1 : (x > varPermMax ? 0 : Math.max(0, Math.min(1, (varPermMax - x) / (varPermMax - varPermMin))))
        );
        permNaikData.push(
            x < varPermMin ? 0 : (x > varPermMax ? 1 : Math.max(0, Math.min(1, (x - varPermMin) / (varPermMax - varPermMin))))
        );
        permInputLine.push(
    ...
    // Persediaan juga mulai dari 0 sampai max+margin
    const persLabels = [];
    ...
    const persStart = 0;
    const persEnd = varPersMax + (varPersMax - varPersMin) * 0.05;
    ...
    for (let x = persStart; x <= persEnd; x += stepPers) {
        ...
        persSedikitData.push(
            x < varPersMin ? 1 : (x > varPersMax ? 0 : Math.max(0, Math.min(1, (varPersMax - x) / (varPersMax - varPersMin))))
        );
        persBanyakData.push(
            x < varPersMin ? 0 : (x > varPersMax ? 1 : Math.max(0, Math.min(1, (x - varPersMin) / (varPersMax - varPersMin))))
        );
        persInputLine.push(
...
function updateCharts() {
    ...
    const nilaiR1 = parseFloat(document.getElementById("nilaiR1").textContent || 0);
    const nilaiR2 = parseFloat(document.getElementById("nilaiR2").textContent || 0);
    const nilaiR3 = parseFloat(document.getElementById("nilaiR3").textContent || 0);
    const nilaiR4 = parseFloat(document.getElementById("nilaiR4").textContent || 0);
